            ...
            actions={
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                {isOn('workflows.finalOutputMode') && (
                  <Button size="sm" variant="ghost" onClick={() => setFinalOutputMode(m => m === 'md' ? 'raw' : 'md')} title={finalOutputMode === 'md' ? 'Show raw output' : 'Show markdown'} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    {finalOutputMode === 'md' ? (
                      ...
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 7V4h16v3" /><path d="M9 20h6" /><path d="M12 4v16" /></svg>
                    )}
                    {!isMobile && <span>Raw</span>}
                  </Button>
                )}
                <Button size="sm" variant="ghost" onClick={() => void copyResult()} disabled={!runFinal} title="Copy output" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                ...
                <Button size="sm" variant="ghost" onClick={() => void sendToChat()} disabled={!runFinal} title="Send to chat" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                ...
                <Button size="sm" variant="secondary" onClick={openSendGitHub} disabled={!runFinal} title="Create GitHub issue" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 .5C5.65.5.5 5.65.5 12c0 5.08 3.29 9.39 7.86 10.92.58.1.79-.25.79-.56v-2.13c-3.2.7-3.87-1.36-3.87-1.36-.52-1.34-1.28-1.7-1.28-1.7-1.05-.72.08-.7.08-.7 1.16.08 1.77 1.2 1.77 1.2 1.03 1.77 2.7 1.26 3.36.96.1-.75.4-1.26.73-1.55-2.55-.29-5.24-1.28-5.24-5.7 0-1.26.45-2.29 1.18-3.1-.12-.29-.51-1.46.11-3.05 0 0 .97-.31 3.18 1.18a11 11 0 015.79 0c2.2-1.49 3.17-1.18 3.17-1.18.62 1.59.23 2.76.12 3.05.73.81 1.18 1.84 1.18 3.1 0 4.43-2.7 5.41-5.27 5.69.41.36.78 1.06.78 2.14v3.18c0 .31.21.67.8.55C20.21 21.39 23.5 17.08 23.5 12 23.5 5.65 18.35.5 12 .5z" /></svg>
  ...
  return (
    <Card style={style}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, userSelect: 'none' }}>
        ...
        {actions != null && <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>{actions}</div>}
      </div>
