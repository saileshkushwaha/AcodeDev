...
const DEFAULT_INPUT =
  'The team shipped a new feature for the dashboard. It includes streaming responses and a new provider selector. Users can now switch between multiple LLM providers from one screen.';
...
export function WorkflowsScreen({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const [finalOutputMode, setFinalOutputMode] = useState<'md' | 'raw'>(isOn('workflows.finalOutputMode') ? 'md' : 'md');
    ...
    (id: string) => {
      ...
      if (Array.isArray(draft?.nodes))
        draft.nodes.forEach((n) => {
          const seq = /_(\d+)$/.exec(n.id);
          if (seq) nodeSeq = Math.max(nodeSeq, Number(seq[1]));
        });
      setActiveId(def.id);
  ...
  const addNode = (type: WorkflowNode['type']) => {
    ...
    if (type === 'llm')
      config = { provider: 'openrouter', model: 'nvidia/nemotron-3.5-lightning:free', systemPrompt: '', temperature: 0.7 };
    ...
    setEdges((es) => {
      const dup = es.some(
        (e) =>
          ...
          (e.sourceHandle ?? 'out') === (edge.sourceHandle ?? 'out') &&
          (e.targetHandle ?? 'in') === (edge.targetHandle ?? 'in'),
      );
  ...
  const pushRun = useCallback((rec: WorkflowRunRecord) => {
  ...
  const runWorkflow = async () => {
          ...
          [
            {
              nodeId: 'err',
              nodeType: 'error',
              output: `⚠️ ${pdef?.name ?? p} isn't connected. Add an API key (Connections → Keys) or pick a key-free provider such as OpenCode Zen or Kilo Gateway.`,
              durationMs: 0,
              status: 'error',
            },
          ],
    ...
    try {
      ...
      pushRun(
        makeRec(
          [{ nodeId: 'err', nodeType: 'error', output: e instanceof Error ? e.message : String(e), durationMs: 0, status: 'error' }],
          '',
        ),
      );
    } finally {
  ...
  const runTokens = useMemo(
    () => runResults?.reduce((s, r) => s + (r.tokens?.prompt ?? 0) + (r.tokens?.completion ?? 0), 0) ?? 0,
    [runResults],
  );
  ...
  const collapseAllResults = useCallback(
    () => setCollapsed(Object.fromEntries((runResults ?? []).map((r) => [r.nodeId, true]))),
    [runResults],
  );
  ...
  return (
    <Page maxWidth={1200}>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" onClick={() => addNode('llm')}>
              + LLM
            </Button>
            <Button variant="secondary" onClick={() => addNode('transform')}>
              + Transform
            </Button>
            <Button variant="secondary" onClick={() => addNode('condition')}>
              + Condition
            </Button>
            <Button variant="secondary" onClick={() => addNode('prompt_template')}>
              + Template
            </Button>
            <Button variant="secondary" onClick={() => addNode('trigger')}>
              + Trigger
            </Button>
            <Button variant="secondary" onClick={() => addNode('http')}>
              + HTTP
            </Button>
            <Button variant="secondary" onClick={() => addNode('fetch')}>
              + Fetch
            </Button>
            <Button variant="secondary" onClick={() => addNode('kv')}>
              + KV
            </Button>
            <Button variant="secondary" onClick={() => addNode('variables')}>
              + Variables
            </Button>
            <Button variant="secondary" onClick={() => addNode('secret')}>
              + Secret
            </Button>
            <Button variant="secondary" onClick={() => addNode('parallel')}>
              + Parallel
            </Button>
            <Button variant="secondary" onClick={() => addNode('loop')}>
              + Loop
            </Button>
      ...
      <Card title="Workflow" subtitle="Pick a preset, or save your current graph as a reusable workflow" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <Button size="sm" onClick={saveDef}>
              💾 Save
            </Button>
            <Button size="sm" variant="secondary" onClick={() => loadDef(BLANK_ID)}>
              ＋ Blank
            </Button>
            {activeIsBuiltin ? (
              <Button size="sm" variant="ghost" onClick={resetDef}>
                ↺ Reset preset
              </Button>
            ) : (
              <Button size="sm" variant="danger" onClick={deleteDef}>
                🗑 Delete
              </Button>
            )}
            <Badge color={activeIsBuiltin ? tokens.primary : undefined}>
              {activeIsBuiltin ? `${catIcon(activeDef?.category)} preset · ${(activeDef?.tags ?? []).join(' · ')}` : 'custom'}
            </Badge>
            <span style={{ fontSize: 12, color: tokens.textMuted }}>
              {presetCount} presets · {customCount} saved
            </span>
          </div>
        </div>
      </Card>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: isMobile ? '1fr' : viewMode === 'graph' ? '2fr 1fr' : '1fr 1fr',
          gap: 16,
          alignItems: 'stretch',
        }}
      >
        <Card
          ...
          subtitle={
            viewMode === 'graph'
              ? 'Canvas · drag to arrange, connect handles to link steps, click a node to edit'
              : 'Runs top → bottom · up/down reorders, duplicate, delete · click to edit'
          }
          actions={
            <span style={{ fontSize: 12, color: tokens.textMuted, display: 'inline-flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              >
                <button
                  ...
                  style={{
                    padding: '4px 10px',
                    border: 'none',
                    background: viewMode === 'list' ? tokens.primary : 'transparent',
                    color: viewMode === 'list' ? tokens.primaryForeground : tokens.textMuted,
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: 'pointer',
                ...
                <button
                  ...
                  style={{
                    padding: '4px 10px',
                    border: 'none',
                    background: viewMode === 'graph' ? tokens.primary : 'transparent',
                    color: viewMode === 'graph' ? tokens.primaryForeground : tokens.textMuted,
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: 'pointer',
                    fontFamily: tokens.fontSans,
              ...
              {viewMode === 'graph' && (
                <Button size="sm" variant="ghost" onClick={autoArrange} title="Auto-arrange nodes into a tidy flow">
                  Auto-arrange
                </Button>
              )}
              <span>
                ...
                return (
                  <div key={n.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <RowBtn onClick={() => moveNode(n.id, 'up')} disabled={!canUp} title="Move up">
                        <Icon name="chevronUp" size={14} />
                      </RowBtn>
                      <RowBtn onClick={() => moveNode(n.id, 'down')} disabled={!canDown} title="Move down">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <RowBtn
                        ...
                        disabled={isTerminal}
                        title={isTerminal ? 'Entry/exit nodes can’t be duplicated' : 'Duplicate'}
                      >
                        <Icon name="copy" size={14} />
                      </RowBtn>
                      <RowBtn onClick={() => deleteNode(n.id)} danger title="Delete">
                        <Icon name="x" size={14} />
                      </RowBtn>
                    </div>
                  </div>
        ...
        >
          {(() => {
            const focus = nodes.find((n) => n.id === selectedNodeId) ?? nodes.find((n) => n.type === 'llm') ?? nodes[0];
            return focus ? (
              <NodeConfig
                node={focus}
      ...
      {runResults && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <Button size="sm" variant="ghost" onClick={collapseAllResults}>
              Collapse all
            </Button>
          </div>
          {runResults.map((r) => (
            <CollapsibleCard
              ...
              actions={
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  {r.status ? (
                    <Badge color={r.status === 'error' ? tokens.danger : tokens.success}>
                      {r.status === 'error' ? '✕ failed' : '✓ ok'}
                    </Badge>
                  ) : undefined}
            ...
            >
              <pre
                style={{
                  ...
                  color: r.status === 'error' ? tokens.danger : tokens.text,
                  maxHeight: 320,
                  overflow: 'auto',
                }}
              >
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                {isOn('workflows.finalOutputMode') && (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setFinalOutputMode((m) => (m === 'md' ? 'raw' : 'md'))}
                    title={finalOutputMode === 'md' ? 'Show raw output' : 'Show markdown'}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}
                  >
                    {finalOutputMode === 'md' ? (
                      <Icon name="code" size={14} strokeWidth={1.8} />
                    ) : (
                      <Icon name="fileCode" size={14} strokeWidth={1.8} />
                    )}
                    {!isMobile && <span>Raw</span>}
                  </Button>
                )}
                <Button
                  size="sm"
                ...
                <Button
                  ...
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}
                >
                  <Icon name="message" size={14} strokeWidth={1.8} />
                  {!isMobile && <span>Send to chat</span>}
                </Button>
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={openSendGitHub}
                  disabled={!runFinal}
                  title="Create GitHub issue"
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}
                >
                  <GithubIcon size={14} />
                  {!isMobile && <span>Create GitHub issue</span>}
                </Button>
              </span>
            }
          >
            {finalOutputMode === 'md' ? (
              <div style={{ color: tokens.success }}>
                {runFinal ? (
                  <Markdown content={runFinal} />
                ) : (
                  <span style={{ fontFamily: tokens.fontMono, fontSize: 13, color: tokens.textMuted }}>(no output)</span>
                )}
              </div>
            ) : (
              <pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontSize: 13, fontFamily: tokens.fontMono, color: tokens.success }}>
                {runFinal || '(no output)'}
              </pre>
            )}
          </CollapsibleCard>
        ...
        <Modal open onClose={() => setGhModal(false)} title="Create GitHub issue" width={560}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ fontSize: 12, color: tokens.textMuted, lineHeight: 1.5 }}>
              The workflow's final output will be attached as the issue body. Note: actions from built-in presets are not executed
              end-to-end against your GitHub repo unless you have a token connected.
            </div>
            {ghMsg && <div style={{ fontSize: 13, color: ghMsg.startsWith('✓') ? tokens.success : tokens.danger }}>{ghMsg}</div>}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <Button size="sm" variant="ghost" onClick={() => setGhModal(false)}>
                Cancel
              </Button>
  ...
  return (
    <Card style={style}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, userSelect: 'none' }}>
        ...
        {actions != null && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>{actions}</div>
        )}
      </div>
...
const RowBtn = React.memo(function RowBtn({
  onClick,
  disabled,
  title,
  danger,
...
const NodeConfig = React.memo(function ({
  node,
  step,
  updateConfig,
  rename,
  ...
  const models = useMemo(
    () => listModels(nodeProvider).map((m) => ({ label: `${m.name}${m.isFree ? ' · free' : ''}`, value: m.id })),
    [nodeProvider],
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div>
        ...
        {runResult ? (
          <div style={{ border: `1px solid ${st === 'error' ? tokens.danger : tokens.success}`, borderRadius: 10, overflow: 'hidden' }}>
            ...
            >
              <span style={{ color: st === 'error' ? tokens.danger : tokens.success, fontWeight: 700, fontSize: 12 }}>
                {st === 'error' ? '✕ Failed' : '✓ OK'}
              </span>
            ...
            <pre
              style={{
                ...
                fontSize: 13,
                fontFamily: tokens.fontMono,
                color: st === 'error' ? tokens.danger : tokens.text,
                maxHeight: 180,
                overflow: 'auto',
              }}
            >
          ...
          <div style={{ fontSize: 13, color: tokens.textMuted, padding: '8px 0' }}>
            No run yet — press Run to see this step's output here.
          </div>
        )}
      </div>
      {node.type === 'llm' && (
        <>
          ...
          <Input
            ...
            onChange={(v) => updateConfig(node.id, 'systemPrompt', v)}
          />
          <Input
            label="Temperature"
            type="number"
            value={String(node.config.temperature ?? 0.7)}
      ...
      {node.type === 'transform' && (
        <Select
          ...
          onChange={(v) => updateConfig(node.id, 'operation', v)}
          options={[
      ...
      {node.type === 'condition' && (
        <Input
          ...
          onChange={(v) => updateConfig(node.id, 'expression', v)}
          hint="Use upstream / input, e.g. upstream.length > 100"
        />
      )}
      {node.type === 'prompt_template' && (
        <Input
          ...
          onChange={(v) => updateConfig(node.id, 'template', v)}
          hint="Use {{input}} / {{upstream}}"
        />
      )}
      {node.type === 'input' && (
        <Input
          label="Default input template"
          textarea
          rows={2}
          value={String(node.config.value)}
          onChange={(v) => updateConfig(node.id, 'value', v)}
        />
      )}
      {node.type === 'trigger' && (
        <>
          <Select
            label="Trigger type"
            value={String(node.config.type)}
            onChange={(v) => updateConfig(node.id, 'type', v)}
            options={[
          ...
          <Input
            ...
            value={String(node.config.expression)}
            onChange={(v) => updateConfig(node.id, 'expression', v)}
            hint="e.g. upstream.length > 100"
          />
          <Input
            label="Interval (ms)"
            type="number"
            value={String(node.config.interval ?? 60000)}
            onChange={(v) => updateConfig(node.id, 'interval', Number(v))}
          />
        </>
      )}
      {node.type === 'http' && (
        <>
          <Select
            ...
            onChange={(v) => updateConfig(node.id, 'method', v)}
            options={[
              { label: 'GET', value: 'GET' },
          ...
          <Input
            label="Headers (JSON)"
            rows={2}
            value={String(JSON.stringify(node.config.headers ?? {}))}
            onChange={(v) => {
              try {
                updateConfig(node.id, 'headers', JSON.parse(v));
              } catch {
                /* ignore */
              }
            }}
      ...
      {node.type === 'fetch' && <Input label="URL" value={String(node.config.url)} onChange={(v) => updateConfig(node.id, 'url', v)} />}
      {node.type === 'kv' && (
        <>
          <Select
            label="Action"
            value={String(node.config.action)}
            onChange={(v) => updateConfig(node.id, 'action', v)}
            options={[
              { label: 'Get', value: 'get' },
              { label: 'Set', value: 'set' },
              { label: 'Delete', value: 'delete' },
          ...
          {node.config.action === 'set' && (
            <Input label="Value" rows={2} value={String(node.config.value ?? '')} onChange={(v) => updateConfig(node.id, 'value', v)} />
          )}
        </>
      )}
      {node.type === 'variables' && (
      ...
      {node.type === 'loop' && (
        <>
          ...
          <Input
            ...
            monospace
            value={String(node.config.body ?? 'upstream')}
            onChange={(v) => updateConfig(node.id, 'body', v)}
            hint="e.g. upstream.task"
          />
