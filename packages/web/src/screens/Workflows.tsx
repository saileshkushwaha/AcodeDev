      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button onClick={runWorkflow} disabled={running}>{running ? <Spinner size={16} /> : <><Icon name="play" size={16} /> Run</>}</Button>
          </>
        }
      />
  ...
  return (
    <Card style={style}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, userSelect: 'none' }}>
        <span
          onClick={onToggle}
          style={{ display: 'inline-flex', alignItems: 'center', transform: expanded ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s', color: tokens.textSecondary, flexShrink: 0, cursor: 'pointer' }}
        >
