...
import { listModels, getFreeModels, listProviders, type ProviderId } from '@acode/core';
...
export function Dashboard({ onNavigate, onOpenEvaluations }: { onNavigate: (id: string) => void; onOpenEvaluations?: () => void }) {
  ...
  const connectedProviders = useMemo(
    () => providers.filter((p) => hasKey(p.id as ProviderId) || p.id === 'local').length,
    [providers, hasKey],
  );
  ...
  const unconfiguredProviders = useMemo(
    () =>
      providers
        .filter((p) => !hasKey(p.id as ProviderId) && p.id !== 'local')
                ...
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: tokens.space3 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {p.gateway && (
                        <Badge style={{ marginLeft: 6, fontSize: 10 }} color={tokens.info}>
                          gateway
                        </Badge>
                      )}
                    </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.space4 }}>
          <Card title="Quick actions">
            <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.space2 }}>
              <Button variant="secondary" full onClick={() => onNavigate('chat')}>
                💬 New chat
              </Button>
              <Button variant="secondary" full onClick={() => onNavigate('workflows')}>
                <Icon name="zap" size={14} /> New workflow
              </Button>
              <Button variant="secondary" full onClick={() => onNavigate('agents')}>
                🤖 New agent
              </Button>
              <Button variant="secondary" full onClick={() => (onOpenEvaluations ? onOpenEvaluations() : onNavigate('prompts'))}>
                🧪 Run eval
              </Button>
            </div>
          </Card>
          <Card title="Providers to configure" subtitle="Connect keys to unlock free models">
            {unconfiguredProviders.map((p) => (
              <div
                key={p.id}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `${tokens.space1}px 0` }}
              >
                <span style={{ fontSize: tokens.fontSizeSm, color: tokens.textSecondary }}>
                  {p.name}
                  {p.gateway && (
                    <Badge style={{ fontSize: 9, marginLeft: 5 }} color={tokens.info}>
                      gateway
                    </Badge>
                  )}
                </span>
                <Button size="sm" variant="ghost" onClick={() => onNavigate('keys')}>
                  Add
                </Button>
              </div>
            ))}
