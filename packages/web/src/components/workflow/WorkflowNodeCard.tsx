...
export const WorkflowNodeCard = memo(
  function WorkflowNodeCard({ id, data, selected }: NodeProps<WorkflowNodeData>) {
    ...
    return (
      <div style={shell}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {result && (
              <span
                style={{
                  fontSize: 11,
                  color: result.status === 'error' ? tokens.danger : tokens.success,
                  fontWeight: 700,
                  whiteSpace: 'nowrap',
                }}
              >
        ...
        {!statusColor && <div style={{ marginTop: 4, fontSize: 11, color: tokens.textMuted }}>—</div>}
        {statusColor && (
